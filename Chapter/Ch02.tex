\chapter{初等関数}\label{chap:function}
...
\end{definition}
\footnotetext[3]{$\left[a, b\right)$を右半開区間，$\left(a, b\right]$を左半開区間と呼び，両者をまとめて半開区間と呼ぶ．また，開区間・半開区間・閉区間をまとめて，区間と呼ぶ．}
\begin{rem*}
...
\begin{figure}[!h]
	...
	\fill[black, name intersections={of=circle and op}] (intersection-1) circle [x radius=.5mm, y radius=.5mm, rotate=0] node[anchor=west] {P=$(\cos\theta, \sin\theta)$};
	...
	\fill[black, name intersections={of=op and ot}] (intersection-1) circle [x radius=.5mm, y radius=.5mm, rotate=0] node[anchor=west] {T=$(1, \tan\theta)$};
	\end{axis}	
	\end{tikzpicture}
	\caption{三角関数の定義}
...
\begin{figure}[!h]
	...
	\begin{subfigure}[t]{.5\textwidth}
		...
		\addplot[dashed, samples=200, domain = -2:2]({-pi/2},{x});
		\addplot[dashed, samples=200, domain = -2:2]({pi/2},{x});
		\addplot[dashed, samples=200, domain = -2:2]({3*pi/2},{x});
		\end{axis}	
		\end{tikzpicture}
...
% % % 例題を入れる
\begin{example*}
	例えば，$\SI{105}{\degree} = \SI{60}{\degree} + \SI{45}{\degree}$であるため，
	\begin{align*}
		\sin\SI{105}{\degree} &= \sin\left(\SI{60}{\degree} + \SI{45}{\degree}\right) \\
		&= \sin\SI{60}{\degree}\cos\SI{45}{\degree} + \cos\SI{60}{\degree}\sin\SI{45}{\degree} \\
